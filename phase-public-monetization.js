(function(){
  ...
  function css(){
    ...
    const s=document.createElement('style');s.id='notulensiAffiliateStyle';s.textContent=`
      ...
      .nm-aff-card{padding:11px;border:1px solid rgba(100,116,139,.25);border-radius:9px;background:#020617;min-width:0}
      ...
      .nm-aff-card a,.nm-aff-card button{display:inline-flex;margin-top:8px;padding:7px 10px;border-radius:7px;border:1px solid #334155;background:#1e293b;color:#dbeafe;text-decoration:none;font:600 10px Inter,system-ui,sans-serif;cursor:pointer}
      .nm-aff-card a:hover,.nm-aff-card button:hover{background:#334155}
      .nm-aff-card button[disabled]{opacity:.55;cursor:not-allowed}
      ...
      .nm-aff-status{display:block;margin-top:8px;color:#fbbf24;font:10px/1.4 Inter,system-ui,sans-serif}
  ...
  function getItems(){const aff=window.NOTULENSI_MONETIZATION?.affiliate||{};return (aff.items||[]).filter(x=>x).slice(0,3)}
  function affiliateBlock(parent){
    ...
    const grid=box.querySelector('.nm-affiliate-grid');
    items.forEach(x=>{
      const card=document.createElement('div');card.className='nm-aff-card';
      const title=document.createElement('b');title.textContent=x.title||'Rekomendasi';
      const desc=document.createElement('span');desc.textContent=x.description||'';
      card.append(title,desc);
      if(x.url){
        const a=document.createElement('a');a.href=x.url;a.target='_blank';a.rel='nofollow sponsored noopener';a.dataset.affiliateId=x.id||'';a.textContent=x.label||'Lihat rekomendasi';card.appendChild(a);
      }else{
        const b=document.createElement('button');b.type='button';b.disabled=true;b.textContent='Menunggu konfigurasi affiliate';card.appendChild(b);
        const st=document.createElement('small');st.className='nm-aff-status';st.textContent='Link affiliate belum dikonfigurasi oleh Admin.';card.appendChild(st);
      }

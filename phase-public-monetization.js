(function(){
  ...
  function css(){
    ...
    s.textContent=`
      ...
      @media(max-width:900px){.nm-affiliate-grid{grid-template-columns:1fr 1fr}}
      @media(max-width:560px){.nm-affiliate-grid{grid-template-columns:1fr}}
    `;
    document.head.appendChild(s);
  }

  const ADVANCED_MODULES = [
    'intelTab','dashboardTab','crossMeetingTab','continuityTab','knowledgeGraphTab','reportTab','docsTab','historyTab'
  ];

  function getItems(){
    return (AFF.items||[]).filter(x=>x&&x.url).slice(0,3);
  }
  ...
  function affiliateBlock(parent){
    ...
    box.id='nmAffiliateBlock';
    box.className='nm-affiliate';
    ...
    items.forEach(x=>{
      ...
      card.className='nm-aff-card';
      card.innerHTML=`<b>${esc(x.title)}</b><span>${esc(x.description)}</span><a href="${esc(x.url)}" target="_blank" rel="nofollow sponsored noopener" data-affiliate-id="${esc(x.id)}">${esc(x.label||'Lihat di Shopee')}</a>`;
      grid.appendChild(card);
    });
    parent.appendChild(box);
  }

  function decorateAdvancedModules(){
    css();
    ADVANCED_MODULES.forEach(id=>affiliateBlock(document.getElementById(id)));
  }

  // Affiliate recommendations are always optional. They never intercept, block,
  // redirect, or unlock application features through a forced click.
  window.NOTULENSI_MONETIZATION_API={
    refresh:decorateAdvancedModules,

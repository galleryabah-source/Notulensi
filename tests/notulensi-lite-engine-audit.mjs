...
const serverAi=fs.readFileSync(new URL('../server/ai-config.js',import.meta.url),'utf8');
const checks=[
 ...
 ['Recording finalization is awaitable',recorder.includes('return new Promise')&&recorder.includes('state.recorder.stop()')],
 ['Recording-session linkage persisted',app.includes('recordingId:audio?.id||state.recordingId||null')],
 ['Reload recovers local audio links',app.includes('recoverAudioLinks')&&app.includes('recorder.getRecording(s.recordingId)')],
 ...
 ['Server AI config uses canonical admin auth',serverAi.includes("../api/_admin-auth.js")&&serverAi.includes('role:session.role')],

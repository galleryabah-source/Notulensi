...
function seed() {
  for (const [id, role, ownerId] of [
    ['admin-1', 'ADMIN', 'owner-a'],
    ['editor-1', 'EDITOR', 'owner-a'],
    ['reviewer-1', 'REVIEWER', 'owner-a'],
    ['auditor-1', 'AUDITOR', 'owner-a'],
    ['owner-1', 'OWNER', 'owner-a'],
    ['owner-b-1', 'OWNER', 'owner-b']
...
function request(method, path, body, cookie) {
  return new Promise((resolve, reject) => {
    const req = require('node:http').request({ hostname: '127.0.0.1', port, path, method, headers: { 'content-type': 'application/json', ...(cookie ? { cookie } : {}) } }, res => {
      let data = ''; res.on('data', c => data += c); res.on('end', () => resolve({ status: res.statusCode, body: JSON.parse(data || '{}'), cookie: res.headers['set-cookie']?.[0]?.split(';')[0] }));
    });
...
async function main() {
  ...
  const adminResource = await request('GET', '/resource?ownerId=owner-b', null, adminCookie); checks.push(['admin authorized', adminResource.status === 200]);
  ...
  const logout = await request('POST', '/logout', null, editor.cookie); const afterLogout = await request('GET', '/resource?ownerId=owner-a', null, editor.cookie); checks.push(['revocation', logout.status === 200 && afterLogout.status === 401]);
  const expiry = await request('POST', '/login', { userId: 'reviewer-1', password: 'test-reviewer-1' }); await new Promise(r => setTimeout(r, SESSION_TTL_MS + 100)); const afterExpiry = await request('GET', '/resource?ownerId=owner-a', null, expiry.cookie); checks.push(['session expiry', afterExpiry.status === 401]);

...
function runtimeCheckFromFile(path, id, phase, name, fallbackDetails) {
  ...
  try {
    ...
    if (value.status === 'PASS') return { id, phase, name, status:'PASS', evidence:[path], details:`Runtime harness passed: ${path}.` };
...
for (const [id,phase,name,details,path] of runtimeDomains) checks.push(runtimeCheckFromFile(path,id,phase,name,details));

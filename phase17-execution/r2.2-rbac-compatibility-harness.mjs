...
check(
  'Current role vocabulary declared',
  JSON.stringify(PHASE17_ROLES) === JSON.stringify(['OWNER', 'ADMIN', 'EDITOR', 'REVIEWER', 'AUDITOR']),
  `roles=${PHASE17_ROLES.join(',')}`
);
...
for (const role of PHASE17_ROLES) {
  ...
  check(
    `Authenticated ${role} allowed when policy permits`,
    allowed.allowed === true && allowed.status === 'AUTHORIZED',
    JSON.stringify(allowed)
  );

  const denied = authorizePhase17({ authenticated: true, role, resourceOwned: true, policyAllows: false });
  check(
    `${role} denied when policy denies`,
    denied.allowed === false && denied.status === 'AUTHORIZATION_DENIED',
...
check(
  'Legacy role string is not silently accepted',
  unknown.allowed === false && unknown.checks.roleKnown === false,
...
check(
  ...
  unauthenticated.allowed === false && unauthenticated.checks.authenticated === false,

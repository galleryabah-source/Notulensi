...
const checks={candidateHealthy:baseline.status===200,canaryTrafficObserved:canaryTraffic===10,canaryFailureDetected,rollbackTriggered,rollbackHealthy:rollback.status===200,rolledBackVersion:rollback.body?.version==='candidate-v1',postRollbackHealthy:postRollback.every(x=>x.status===200&&x.body?.version==='candidate-v1')};
const status=Object.values(checks).every(Boolean)?'PASS':'FAIL';
const report={schemaVersion:'1.1.0',status,environment:'controlled-ci-runtime',checks,release:{candidate:'candidate-v2',rollbackTarget:'candidate-v1',canaryRequests:10,postRollbackRequests:10},scope:'controlled canary/rollback drill; production deployment traffic-shifting evidence remains separately gated'};

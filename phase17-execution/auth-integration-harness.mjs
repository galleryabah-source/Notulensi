...
function request(path, options = {}) {
  ...
  const split = raw.split(/\r?\n\r?\n/); const headers = split.shift() || ''; const body = split.join('\n\n');
  const status = Number((headers.match(/HTTP\/\d(?:\.\d)?\s+(\d+)/) || [])[1] || 0);
...
function sleep(ms) { execFileSync('sleep', [String(Math.ceil(ms / 1000))]); }
...
try {
  ...
  const ownerBLogin = request('/api/auth/login', { method: 'POST', body: { username: 'viewer-b', password: 'phase17-viewer-b-password' } });
  const ownerB = request('/api/protected/owner-a', { cookie: ownerBLogin.cookie });
  checks.push(check('Cross-owner access denied', ownerBLogin.status === 200 && ownerB.status === 403, `login=${ownerBLogin.status}; ownerA=${ownerB.status}`));
  const expiringLogin = request('/api/auth/login', { method: 'POST', body: { username: 'viewer', password: 'phase17-viewer-password' } });
  sleep(2000);
  const expired = request('/api/auth/me', { cookie: expiringLogin.cookie });
  checks.push(check('Session expiry enforced', expiringLogin.status === 200 && expired.status === 401, `login=${expiringLogin.status}; afterExpiry=${expired.status}`));
...
} catch (error) { checks.push(check('Harness execution', false, error.message)); }
const passed = checks.every((item) => item.status === 'PASS');
const report = {
  schemaVersion: '1.0.0', checkId: '17-E.runtime.auth.integration', phase: '17-E.11-B',
  name: 'Executable authentication/RBAC integration boundary harness', status: passed ? 'PASS' : 'FAIL',
  applicationRuntimeIntegrated: false, integrationFixture: true, generatedAt: new Date().toISOString(), checks,
  details: 'CI-only isolated fixture. Not application-runtime evidence until the real application server delegates to this boundary and integrationFixture is removed.'
};

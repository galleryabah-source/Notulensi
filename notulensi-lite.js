(() => {
  ...
  // Persistence is an adapter boundary; production DB integration is gated by audit.
  ...
  function save() { store.write({ meetings: state.meetings, transcripts: state.transcripts }); render(); }
  function id(prefix) { return `${prefix}_${Date.now()}_${Math.random().toString(36).slice(2,8)}`; }
  const esc = s => String(s ?? '').replace(/[&<>'"]/g, c => ({'&':'&amp;','<':'&lt;','>':'&gt;',"'":'&#39;','"':'&quot;'}[c]));
  function show(view) { document.querySelectorAll('.view').forEach(x => x.classList.add('hidden')); const target=document.getElementById(view); if(target)target.classList.remove('hidden'); document.querySelectorAll('.nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===view)); }
  function render() {
    document.getElementById('meetingCount').textContent=state.meetings.length; document.getElementById('transcriptCount').textContent=state.transcripts.length;
    document.getElementById('meetingList').innerHTML=state.meetings.length?state.meetings.map(m=>`<article class="item" data-meeting="${m.id}"><span class="badge">Rekap</span><strong> ${esc(m.title||'Tanpa judul')}</strong><div class="muted">${esc(m.date||'')}</div><div>${esc(m.summary||m.agenda||'').slice(0,180)}</div></article>`).join(''):'<div class="empty">Belum ada rekap.</div>';
    document.getElementById('transcriptList').innerHTML=state.transcripts.length?state.transcripts.map(t=>`<article class="item" data-transcript="${t.id}"><span class="badge">Transkrip</span><strong> ${esc(t.title||'Tanpa judul')}</strong><div>${esc(t.body||'').slice(0,220)}</div></article>`).join(''):'<div class="empty">Belum ada transkrip.</div>';
  }
  ...
  document.getElementById('saveMeeting').onclick=()=>{const m={id:id('m'),title:document.getElementById('mTitle').value.trim(),date:document.getElementById('mDate').value,participants:document.getElementById('mParticipants').value.trim(),agenda:document.getElementById('mAgenda').value.trim(),summary:document.getElementById('mSummary').value.trim(),updatedAt:new Date().toISOString()};if(!m.title){alert('Judul rapat wajib diisi.');return;}state.meetings.unshift(m);save();document.getElementById('meetingEditor').classList.add('hidden');};

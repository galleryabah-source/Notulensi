...
function normalizeDatabaseUrl(value){const raw=String(value||'').trim();if(!raw)return '';try{const u=new URL(raw);if(u.searchParams.has('sslmode'))u.searchParams.set('sslmode','verify-full');return u.toString();}catch{return raw.replace(/([?&]sslmode=)(prefer|require|verify-ca)(?=&|$)/i,'$1verify-full');}}
...
const defaults={defaultProvider:'gemini',providers:{ollama:{model:'qwen2.5:7b',baseUrl:'',key:''},gemini:{model:'gemini-3.6-flash',key:''},groq:{model:'llama-3.3-70b-versatile',key:''},openrouter:{model:'openrouter/free',key:''},huggingface:{model:'meta-llama/Llama-3.1-8B-Instruct',key:''}}};
export async function readConfig(client){const r=await client.query('SELECT config FROM notulensi_ai_config WHERE config_key=$1',['default']);const saved=r.rows[0]?.config||{};return {...defaults,...saved,defaultProvider:process.env.AI_PROVIDER||saved.defaultProvider||defaults.defaultProvider,providers:{...defaults.providers,...(saved.providers||{})}};}

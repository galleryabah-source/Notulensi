...
const PROVIDERS = {
  ...
  groq: { name:'Groq', model:'llama-3.3-70b-versatile', testUrl:'https://api.groq.com/api/v1/models' },
...
function normalizeDatabaseUrl(value){const raw=String(value||'').trim();if(!raw)return '';try{const url=new URL(raw);url.searchParams.delete('sslmode');url.searchParams.delete('uselibpqcompat');return url.toString()}catch{return raw}}
export function db(){const connectionString=normalizeDatabaseUrl(process.env.DATABASE_URL||process.env.notulensi_POSTGRES_URL||process.env.notulensi_POSTGRES_PRISMA_URL||process.env.notulensi_DATABASE_URL_UNPOOLED);if(!connectionString)throw new Error('DATABASE_URL is not configured.');if(!poolHolder.pool){const sslMode=String(process.env.DATABASE_SSL||'verify-full').toLowerCase();poolHolder.pool=new Pool({connectionString,max:2,ssl:sslMode==='disable'?false:{rejectUnauthorized:true}})}return poolHolder.pool;}
...
export default async function handler(req,res){res.setHeader('Cache-Control','no-store');if(req.method!=='GET'&&req.method!=='PUT')return res.status(405).json({error:'Method not allowed'});if(req.method==='GET'){let c;try{const client=await db().connect();await ensureTable(client);const config=await readConfig(client);client.release();return res.status(200).json({config,providers:PROVIDERS});}catch(e){return res.status(503).json({error:e.message||'AI config unavailable'});}}const session=requireAdmin(req,res);if(!session)return;let client;try{client=await db().connect();await ensureTable(client);const current=await readConfig(client);const incoming=req.body?.config||req.body||{};const next={...current,...incoming,providers:{...current.providers,...(incoming.providers||{})}};await client.query(`INSERT INTO notulensi_ai_config(config_key,config,updated_at) VALUES($1,$2::jsonb,NOW()) ON CONFLICT(config_key) DO UPDATE SET config=EXCLUDED.config,updated_at=NOW()`,['default',JSON.stringify(next)]);return res.status(200).json({saved:true,config:next,role:session.role});}catch(e){console.error('ai-config',e);return res.status(503).json({error:e.message||'AI config unavailable'});}finally{client?.release();}}

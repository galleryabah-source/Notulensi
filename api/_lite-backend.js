...
function normalizeMeeting(value, index) {
  return { id: text(value?.id, 120) || `m_${index + 1}`, title: text(value?.title, 240), date: text(value?.date, 40), participants: text(value?.participants, 4000), agenda: text(value?.agenda, 12000), summary: text(value?.summary, 12000), updatedAt: text(value?.updatedAt, 60) };
}
function normalizeTranscript(value, index) {
  return { id: text(value?.id, 120) || `t_${index + 1}`, title: text(value?.title, 240), body: text(value?.body, 50000), updatedAt: text(value?.updatedAt, 60) };
}
